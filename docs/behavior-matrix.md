...
| --- | --- | --- |
| CPU topology | Cluster index plus configured core count | Exact live `related_cpus` masks and dynamic sparse CPU sets; imported `nr` is only a consistency check |
...
| Recovery | Android service/script lifecycle restore | Backward-compatible v6 journal (v5 tagged resources plus exact FIFO priority); failed or ambiguous restore makes every mutation read-only |
...
| Rollout | Controller is always active | `legacy`, observe-only `shadow`, and fail-closed `energy`; uncalibrated models cannot become active defaults |
...
| Rendering | SurfaceFlinger analysis and junk hints | Authenticated GNOME/Mutter paint, presentation deadline, render-idle, and physical display power events |
...
| Interconnect/sysfs | Logical names may resolve to arbitrary Android paths | Root-owned typed scalar targets with closed domains, exact paths, readback, reverse rollback, and v5+ recovery |
...
| Observability | Log level, trace markers, verbose hint transitions | Structured tracing/journald events, monotonic decision/reconcile IDs, bounded D-Bus trace, health diagnostics, and offline replay |
